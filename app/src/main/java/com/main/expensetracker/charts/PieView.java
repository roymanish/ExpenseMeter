package com.main.expensetracker.charts;
...
//        postInvalidate();
    ...
    private void initPies(ArrayList<PieHelper> helperList){
        ...
        for(PieHelper pie:helperList){
            pie.setDegree(totalAngle, totalAngle + pie.getSweep());
            totalAngle += pie.getSweep();
            
        }
        if(totalAngle != 360){
        	PieHelper pie = new PieHelper((360-totalAngle)/totalAngle, "Remaining");
        	pie.setDegree(totalAngle, 360);
        	helperList.add(pie);
        }
    }
